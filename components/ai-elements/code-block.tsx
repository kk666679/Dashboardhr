...
// Highlighter cache (singleton per language, max 50 langs)
const highlighterCache = new Map<
  string,
  Promise<HighlighterGeneric<BundledLanguage, BundledTheme>>
>();

// Token cache (max 200 entries)
const tokensCache = new Map<string, TokenizedCode>();

// Subscribers for async token updates
const subscribers = new Map<string, Set<(result: TokenizedCode) => void>>();

const MAX_HIGHLIGHTERS = 50;
const MAX_TOKENS = 200;

// Cleanup function for caches
function cleanupCaches() {
  // Limit highlighterCache
  ...
  // Limit tokensCache
  if (tokensCache.size > MAX_TOKENS) {
...
export const highlightCode = (
  ...
  getHighlighter(language)
    ...
    .catch((error) => {
      console.error("Failed to highlight code:", String(error).slice(0, 200));
      subscribers.delete(tokensCacheKey);
    });
